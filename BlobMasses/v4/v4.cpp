...
cv::Point2f findCentroidUsingConnectedComponents(const cv::Mat& binaryImage) {
    cv::Mat labels;
    int nLabels = cv::connectedComponents(binaryImage, labels);

    double sumX = 0, sumY = 0, totalWeight = 0;

    for (int y = 0; y < binaryImage.rows; y++) {
        for (int x = 0; x < binaryImage.cols; x++) {
            if (labels.at<int>(y, x) > 0) { // пропускаем фон (метка 0)
                sumX += x;
                sumY += y;
                totalWeight += 1;
            }
...
int main() {
    ...
    for (const auto& entry : fs::directory_iterator(inputFolder)) {
        ...
        cv::Point2f center = findCentroidUsingConnectedComponents(image);
        //std::cout << "Центр масс (метод connectedComponents): " << center << std::endl;
